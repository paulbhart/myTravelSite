...
public class ListController {
	...
	public String getHotelsbyState(@PathVariable String state,Locale locale, Model model, HttpServletRequest HttpServletRequest, @RequestParam(value="format") String format) throws Exception {
		logger.info("Hotels Controller /State" + state);
	
	    ArrayList<Hotel> htls = mySearchService.getHotelsByState(TwoCharState.toEnum(state));
		
		ObjectMapper mapper = new ObjectMapper(); 
		
		String hotels=mapper.writeValueAsString(htls);
		
		hotels = "{ \"ResultSet\": { \"firstResultPosition\": 1, \"totalResultsReturned\": " + htls.size() + ", \"Result\": " + hotels + "}}";
